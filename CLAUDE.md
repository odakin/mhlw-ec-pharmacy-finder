...
docs/                    → GitHub Pages 公開ディレクトリ
  ...
  data.json              → 薬局データ（13,006件。空レコード除外前の厚労省公表件数は13,233件）
...
```json
{
  "meta": { "asOf": "2026-04-06", "records": 13006, "totalPublished": 13233, "scriptHash": "...", ... },
  "data": [
    {
      "id": 66,
      "pref": "北海道",
...
```json
{
  "meta": { "asOf": "2026-03-24", "records": 3105, ... },
  "data": [
    {
      "id": "c-a067f631",
      "pref": "北海道",
...
|---|---|---|---|
| `"有"` / `"あり"` / `"有 薬品名..."` | **常時在庫あり** — いつ行っても院内に薬がある。その場でもらえる | 2,965件（95%） | **表示対象** |
| `"無"` / `"無（院外処方）"` 等 | **常時在庫なし** — 診察・処方はできるが、薬は院内に常備していない。院外処方箋を持って薬局へ行く必要がある | 129件（4%） | **除外**（※1） |
| `""` （空欄） | **未回答・不明** | 8件（0.3%） | **除外** |
...
4. **機能3**: 営業時間表示 — 曜日別パーサー（薬局 98.2% / 医療機関 88.3% カバー率）+ 今日ハイライト + 折りたたみ全曜日表示
