...
このサイトのフィルターには2種類ある: **デフォルト除外**（ユーザーに選択肢を見せない）と**ユーザー起動型**（ユーザーが自分でONにする絞り込み）。それぞれ判断基準が異なる。

### デフォルト除外の判断基準

緊急避妊薬は時間勝負。デフォルトで選択肢を隠す判断は慎重に行い、以下の3軸で評価する:
...
一見どちらも「不確実なものを除外する」構造に見えるが、方向が逆: 在庫フィルターは施設の自己申告を信頼し確度の低い選択肢をノイズとして除去する。営業中フィルターはこちらのパーサーの限界により確度の高い選択肢まで巻き添えで消す。

### ユーザー起動型フィルターの判断基準

ユーザーが自分でONにするフィルター（女性薬剤師、時間外対応、事前連絡不要、個室あり等）は、デフォルト除外より判断基準が緩い。OFFで全件見えるため、ユーザーが自分の意思で絞り込んでいる。必要なのは:

1. **データが一次情報であること** — 施設の自己申告に基づくこと（推定ではないこと）
2. **絞り込み率が意味のある水準であること** — 通過率が高すぎるフィルターは機能しない

**「個室あり」フィルター** → **実装済み**

- データ: 施設自身が申告した `privacy` フィールド（一次情報）
- 絞り込み率: 1,443件/10,128件（14%）。意味のある絞り込み
- **URL**: `&room=1`
- **医療機関**: `clinics.json` に privacy フィールドがないため対象外

**「衝立以上」フィルター**（仮に実装した場合）→ **実装しない**

- 衝立/パーテーションは全薬局の79%（7,968件）が該当。10件中9件が通過するフィルターは機能しない
- 逆方向（「物理的仕切りなし」の除外）で見ても約11%（~1,150件）。カードの「プライバシー」欄で目視確認可能な程度の混在率
- 緊急時にフィルターが多すぎると迷いの原因になる。個室フィルターで強いニーズを満たし、中間ニーズ（衝立でいい）はほとんどの薬局が自然に満たしている状態を維持
