...
セグメント配列（例: ["月-金:9:00-18:00", "土:9:00-13:00"]）
  ↓
parseHours()          ← 各セグメントから曜日+時間帯を抽出
  ↓
構造化データ: { schedule: [{days: [1,2,3,4,5], open: "9:00", close: "18:00"}, ...] }
```
...
**なぜ除去するか**: パーサーの出力は `{days, open, close}` の配列。
「休み」を表現する型がない。残すとパース失敗になり、
本来パースできる営業時間まで道連れで生データ表示になる。
除去すれば営業日だけ構造化でき、休業日は「スケジュールに載っていない＝休み」として暗黙表現される。
