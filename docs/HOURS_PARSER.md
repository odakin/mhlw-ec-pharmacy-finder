...
**保守的適用ルール（applyClosedDays）**: closedDays は**単一 multi-day エントリ内の曜日のみ**除去する。同じ曜日が複数エントリに現れる場合（例: `平日:9:00-18:00` + `木・土:9:30-13:00`）は適用しない — 異なる時間帯を持つ可能性があるため。
...
### 週間スケジュールグリッドの表示順（月曜始まり）

折りたたみ全曜日表示の週間スケジュールグリッドは**月曜始まり**で並べる（月・火・水・木・金・土・日 + 祝日行を最後）。

- 祝日行をグリッドの末尾に置けるため、構造として自然（日曜始まりだと祝日行の位置が中途半端になる）
- 改修履歴の `c3a7f62`（祝日対応・月曜始まり化）で日曜始まりから変更
