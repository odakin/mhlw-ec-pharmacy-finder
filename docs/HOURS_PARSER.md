...
- パーサー全体（正規化〜レンダリング）で約590行、正規表現パターン約150個
...
`holidayClosed` と同じ手法で、rawNorm から除外/休業情報を抽出する（5パターン: 定休日リスト、Xを除く、括弧内除外、X休み/休診、括弧内休診）。

- **closedDays**: 自明な完全休業日（単一エントリ内の曜日除去のみ適用、曖昧な場合は適用しない）
- **hoursNotes**: 全除外情報を注記テキストとして保全（closedDays 適用の有無にかかわらず表示）

原則: **情報は捨てない**。正規化段階で休業日テキストを除去しても、事前抽出により情報は hoursNotes に残る。
...
**設計判断**: **Correct > Correct + caveat > Unknown > Wrong**（DESIGN.md §6 参照）。
「毎週土曜営業」と誤解させる（Wrong）より、注記で「※第1・3土曜のみ」と伝える（Correct + caveat）方が安全。
