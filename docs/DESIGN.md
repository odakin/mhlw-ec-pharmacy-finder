...
祝日の判定は外部API依存ゼロの純粋計算で実装（`getJapaneseHolidays()`、約60行）。固定日・ハッピーマンデー・春分秋分（天文公式）・振替休日・国民の休日すべてに対応し、~2099年まで有効。
...
| 選択 | 理由 |
|---|---|
| **Leaflet.js + OpenStreetMap** | 無料。Google Maps Platform は有料（$200/月の無料枠はあるが、維持管理の負担） |
| **東大CSIS ジオコーディング** | 無料・APIキー不要・番地レベル精度。Nominatim は日本語住所で全滅。Google Geocoding API は同等精度だが有料 |
