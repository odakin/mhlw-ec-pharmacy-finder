...
除外により `data.json` の件数は厚労省公表件数より少なくなる。この2つの数字はそれぞれ別の意味を持つ（2026-04-06時点の例）:

| 数字 | 意味 | 表示場所 |
|---|---|---|
| **13,233** | 厚労省が公表した施設数（データの網羅性） | ハイライトカード「💊 全国 13,233 件」 |
| **13,006** | 実際に検索可能な施設数（ユーザーが触れるデータ） | ステータスバー「13,006件の薬局データを読み込みました」 |
...
「近い順」ソート（GPS）があるため、駅名・地名による検索ボックスは**意図的に実装しない**。

- GPS による「現在地から近い順」で位置に基づく絞り込みのニーズは満たされている
- 駅名・地名検索を実装すると、ジオコーディング API への外部依存（住所文字列→緯度経度の変換）が新たに発生する。**緊急時ツールにとって外部 API 依存は信頼性を下げる障害点**であり、§7「技術選定の原則」の「ネットワーク依存の障害点を増やしたくない」と同じ判断
- ユーザーが手元の現在地ではなく任意地点を起点にしたい場合は、地図ビューを直接パン・ズームして近隣施設を確認できる
...
## 11. 地図ポップアップは簡易表示、詳細はリストカードに委譲する

### 問題

地図ビューでピンをクリックして表示される Leaflet のポップアップは、表示領域が狭く、かつ markercluster の展開状態に左右されるため、**全項目を詰め込むと可読性が崩壊する**。一方で、緊急避妊薬の利用者にとってクリティカルな項目（特に `afterHoursTel` = 時間外対応電話番号、`callAhead` = 事前連絡要否、`notes` = 備考）は省略できない。

### 決定: ポップアップは簡易データのみ、吹き出しクリックでリストカードへジャンプ

- ポップアップには名称・住所・電話・Google Map リンクなどの **識別に必要な最小セット** のみ表示する（現状通り）
- ポップアップ全体（または明示的な「詳細を見る」リンク）をクリックすると、リスト表示に切り替わり、**該当レコードのカードへスクロール + 一時的にハイライト** する動線を追加する
- 詳細データ（`afterHoursTel`, `callAhead`, `notes`, 営業時間全曜日展開, 薬剤師人数など）は**リストカード側の責務**として集約する。ポップアップに同じものを重複実装しない

### なぜポップアップを拡張せず、リスト側に委譲するか

- **単一の情報源:** 詳細カードのレイアウトは既に安定しており、ポップアップ版を別途作ると項目追加のたびに 2 箇所メンテが必要になる
- **可読性:** 狭いポップアップに時間外情報まで詰め込むと、特に医療機関の長い備考で破綻する
- **モバイル UX:** スマホではポップアップ内スクロールは操作性が悪い。フルスクリーンのリストカードに遷移したほうが読みやすい
- **検索・絞り込みとの整合:** リストに飛べば、そのまま隣接カードの比較・再検索に自然につながる

### 却下した代替案

| 案 | 却下理由 |
|---|---|
| ポップアップに全項目を詰め込む | 可読性崩壊、2 箇所メンテ、モバイル NG |
