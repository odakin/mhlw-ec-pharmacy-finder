...
#### 適用例: 「今開いている」フィルター → ~~実装しない~~ ユーザー起動型として実装

当初は「実装しない」と判断していた。しかしパーサー改善（薬局98.2%、医療機関88.3%）と「不明バッジ」設計により、3軸評価が変わった:

- **情報の出所**: パーサー推定（二次加工）だが、パース成功レコードの時間判定は正確。フィルターが隠すのは「パース成功 AND 営業時間外」のみ
- **代替**: パース不能施設は隠さず「営業状況不明」バッジ付きで表示。最寄り施設が消えることはない
- **除外しない場合の害**: 東京都で夜10時 → 50件中40件が営業時間外。モバイルで10件の営業中を探すスクロールは、緊急時に実害がある

→ 「確実に閉まっている」だけを隠し、不確実なもの（パース不能）は残す設計。フィルターが新たなリスクを追加しない。デフォルトOFF（ユーザー起動型）。詳細は [NOW_OPEN_FILTER.md](../NOW_OPEN_FILTER.md) を参照。
...
医療機関の営業時間は薬局とフォーマット傾向が異なる（AM/PM表記、午前/午後、括弧内注記、自由記述が多い）。パーサーは以下の方針で拡張した:

- **括弧内の除外/休診注記**（`（除く水曜）` `（木休診）` 等）は丸ごと除去し、基礎スケジュールを保持する。除外曜日の情報は失われるが、**Missing info > Wrong info** の原則に基づき、月-金を表示する方が土だけ表示するより安全
- **注記セグメント**（`日・祝・GW・お盆・年末年始除く` 等）は graceful skip。時間情報を含まないセグメントがパース全体を道連れにしない
