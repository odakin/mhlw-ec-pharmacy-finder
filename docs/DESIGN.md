...
- **括弧内の除外/休診注記**（`（除く水曜）` `（木休診）` 等）は正規化段階で除去するが、`parseHours` が事前に raw テキストから除外情報を抽出・保全する。**情報は捨てない** — 解釈できるものはスケジュールに反映し、できないものは付加情報（※注記）として表示する。
  ...
  - **旧設計（〜2026-03-31）の反省**: 以前は「**Missing info > Wrong info**」を根拠に除外情報を丸ごと捨てていた。しかし `月-金:9:00-18:00（除く水曜）` から水曜の除外を捨てると、水曜が営業中と表示される — これは Missing ではなく **Wrong**（誤った営業時間の積極的表示）。緊急避妊薬のユースケースでは「行ったら閉まっていた」が深刻な結果を招くため、情報の廃棄は安全側ではなかった
  - **現設計の原則**: **Correct > Correct + caveat > Unknown > Wrong**。除外情報を丸ごと捨てて誤った営業日を表示する（Wrong）より、基盤スケジュール＋注記を表示する（Correct + caveat）方が安全
