...
- **括弧内の除外/休診注記**（`（除く水曜）` `（木休診）` 等）は丸ごと除去し、基礎スケジュールを保持する。除外曜日の情報は失われるが、**Missing info > Wrong info** の原則に基づき、月-金を表示する方が土だけ表示するより安全。「除く水曜」を解析して水曜を引く方法も検討したが、正規化段階で `（除く水）` が曜日:時間のセグメント構造を破壊する問題があり、除去が最も安全
