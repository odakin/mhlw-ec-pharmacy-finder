...
- h1: 「緊急避妊薬（アフターピル）販売可能な薬局検索」
- title/meta: 「緊急避妊薬（アフターピル）薬局検索 | 全国10,000件以上」
- 概要文の「必ず」を削除（「最終確認は公式情報と…」に）
- 免責の「非公式ツール」はそのまま残す

### 「アフターピル」SEO強化（2026-03-21）

- h1を「要指導医薬品」→「アフターピル」に変更（検索ボリュームが圧倒的に高い。要指導医薬品は一般ユーザーに意味不明）
- title/og:title/twitter:title/JSON-LD name を統一: 「緊急避妊薬（アフターピル）薬局検索 | 全国10,000件以上」
- meta description/og:description/twitter:description に「モーニングアフターピル」「処方箋なし」を追加
- header note を「処方箋なしで購入できます」に変更（JS有効ユーザーにも「アフターピル」「処方箋なし」が可視テキストとして出る）
- app.js の BASE_TITLE を同期
- README.md のh1を同期
...
### 「個室あり」フィルター追加（2026-03-21）

- afterpill-map.jp との比較で、プライバシー環境フィルターが有用と判断
- `privacy` フィールドに「個室」を含む薬局に絞り込むチェックボックスを追加（1,443件/10,128件 = 14%）
- 衝立は79%が該当しフィルターとして機能しないため「個室」のみ。詳細は CLAUDE.md「フィルター設計原則」参照
- URL: `&room=1`、FAQ構造化データ+可視FAQ+noscriptに追加
- 医療機関にはprivacyフィールドがないためフィルター対象外
- フィルター設計原則との整合: ユーザー起動型フィルター（OFFで全件見える）。一次情報＋絞り込み率14%で2条件クリア。詳細は CLAUDE.md 参照

### 設計思想ドキュメント追加（2026-03-21）

- `docs/DESIGN.md` + `docs/DESIGN.html` を新規作成
...
- 厚労省データの177件は住所・電話・営業時間すべて空（統合済み117件、振り直し2件、備考なし58件）
- ユーザーに表示する意味がない（電話もできない、住所もない、Google Mapリンクも無意味）
- フィルター設計原則に照らし「デフォルト除外」と判定（一次情報＋代替あり＋ノイズ除去の実益）
- `update_data.py`: addr空レコードを除外（データ変換時）、除外件数をログ出力
- `app.js`: フロントエンド側でも `!rr.addr` を防御フィルターとして追加
- `docs/data.json`: 10,128件→9,951件
- SEO表記: 「全国10,000件以上」はそのまま維持（厚労省データセットの規模を指す表現として正確）
- `data.json` の `meta.totalPublished` に厚労省公表件数（10,128）を保持
- 件数表示の2層構造: ハイライトカードは `totalPublished`（10,128 = データの網羅性）、ステータスバーは `DATA.length`（9,951 = 検索可能件数）
- DESIGN.md / DESIGN_EN.md にセクション3「空レコード除外と件数表示の2層構造」として設計思想を文書化
- ハイライトカード: `META.totalPublished`（10,128件）を表示。「厚労省公式データ全件収録」と整合
- ステータスバー: `DATA.length`（9,951件）を表示。実際に検索できる件数
- 2層の件数表示は別の意味: ハイライト=厚労省データの網羅性、ステータス=検索機能の正確性
...
- [x] **医療機関の常時在庫フィルタ** — `stock` が「有」「あり」で始まるもののみ表示（2,964/3,107件）。在庫なし・不明は除外。バッジ表記を「在庫あり」→「常時在庫あり」に変更。トグルラベル・バナーボタン・FAQ説明すべて「常時在庫ありの医療機関も表示」に統一（🏥絵文字なし、他フィルターと整合）。CLAUDE.md に設計判断とデータ構造を詳細記載
- [x] **出典行に薬局・医療機関のデータ時点を併記** — ヘッダー出典を「厚生労働省（公式）薬局一覧（日付）・医療機関一覧（日付）」に変更。リンク2つ（各公式ページ）。医療機関部分はclinics.jsonロード後に表示
...
- [x] **FAQ「薬局と医療機関の違い」追加** — 可視FAQ・JSON-LD FAQPage・noscript の3箇所に追加。配置検討の結果、FAQ のみが最適と判断（ヘッダー/フィルター周辺は急いでいるユーザーへのノイズになる）
...
- 「現在営業中」フィルターは実装しない（2026-03-21）— 詳細は CLAUDE.md「フィルター設計原則」参照。要点: ①根拠が二次加工（パーサー推定、カバー率97.1%）で約290件が判定不能、②パース不能薬局が最寄りの可能性があり代替なし、③除外しなくても害がない（カード表示で十分）。在庫フィルター（一次情報＋代替あり＋ノイズ除去の実益）とは3軸すべてで条件が異なり、両判断は整合的
- JFPA（jfpa-clinic.org/phs/）との比較検討（2026-03-21）:
  - **駅名・地名検索**: 不要。主要ユースケース（今・ここ・急ぎ）は「近い順」GPSで解決済み。エリア絞り込みは都道府県＋テキスト検索で実用上十分。外部API依存（Google Places等）は緊急時ツールの信頼性を下げる。JFPA は営業時間も電話も距離も出ないから地名検索がないと探せないだけで、土俵が違う
  - **緊急避妊薬の医療情報セクション**: 不要。このサイトは検索ツールであり医療情報提供サイトではない。医療情報の正確性担保は専門機関の仕事
  - **薬局 vs 医療機関の使い分け説明**: FAQ に1問追加で対応。ヘッダー・フィルター周辺への配置は検討の結果却下（急いでいるユーザーの導線にノイズを足すべきでない。デフォルト=薬局で正しい行動が起きる。結果少数時はバナーが医療機関を誘導済み。違いを知りたいユーザーはFAQで対応）
