...
- [x] **祝日対応の実装**（営業時間パーサー拡張）— 実装済み・未コミット
  - 3層構造で実装:
  ...
  - カバー率: 97.1% 維持（変更なし）。祝日スケジュール取得505件、祝日休みフラグ74件
  - 2026年の祝日計算テスト: 全17日正確（振替休日含む）
- [ ] **祝日法改正チェックのスケジュールタスク登録**（祝日対応の実装後）
  - 年1回（例: 1月）、祝日計算ロジックが最新の法律と合っているか確認するリマインダーを設定
