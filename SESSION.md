...
- JFPA（jfpa-clinic.org/phs/）との比較検討（2026-03-21）— 以下すべて不要と判断:
  - **駅名・地名検索**: 主要ユースケース（今・ここ・急ぎ）は「近い順」GPSで解決済み。エリア絞り込みは都道府県＋テキスト検索で実用上十分。外部API依存（Google Places等）は緊急時ツールの信頼性を下げる。JFPA は営業時間も電話も距離も出ないから地名検索がないと探せないだけで、土俵が違う
  - **緊急避妊薬の医療情報セクション**: このサイトは検索ツールであり医療情報提供サイトではない。医療情報の正確性担保は専門機関の仕事
  - **OTC vs 処方箋ルートの説明**: 医療機関トグルのラベル（「常時在庫ありの医療機関も表示」＋「※医師の対面診察・処方箋が必要です」注記）で十分伝わっている
