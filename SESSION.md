...
- [x] **医療機関の常時在庫フィルタ** — `stock` が「有」「あり」で始まるもののみ表示（2,964/3,107件）。在庫なし・不明は除外。バッジ表記を「在庫あり」→「常時在庫あり」に変更。トグルラベル・バナーボタン・FAQ説明すべて「🏥 常時在庫ありの医療機関も表示」に統一。CLAUDE.md に設計判断とデータ構造を詳細記載
